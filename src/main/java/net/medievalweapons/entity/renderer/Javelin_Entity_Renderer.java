...
public class Javelin_Entity_Renderer extends EntityRenderer<Javelin_Entity> {
    ...
    private final Javelin_Entity_Model model = new Javelin_Entity_Model(Javelin_Entity_Model.getTexturedModelData().createModel());
    ...
    public void render(Javelin_Entity javelin_Entity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
        matrixStack.push();
        VertexConsumer vertexConsumer = ItemRenderer.getItemGlintConsumer(vertexConsumerProvider, model.getLayer(this.getTexture(javelin_Entity)), false, javelin_Entity.enchantingGlint());
        ...
        matrixStack.multiply(Vec3f.POSITIVE_Z.getDegreesQuaternion(MathHelper.lerp(g, javelin_Entity.prevPitch, javelin_Entity.getPitch()) + 90.0F));
